...
savename = ['tri_',num2str(trial_num),'_DCM_dfp'];
...
% for z = 1;
    ...
    %  scan parameter
    matlabbatch{X}.spm.stats.fmri_spec.dir = {fullfile(parent_dir,members{z},['results_representation_',num2str(trial_num),'_DCM'])};  %%
    ...
    elseif z>=strmatch('nakamura',members)
        matlabbatch{X}.spm.stats.fmri_spec.sess(h).cond(1).name = 'MI_good';
        ...
        matlabbatch{X}.spm.stats.fmri_spec.sess(h).cond(2).name = 'MI_bad';
...
for z = 1:size(members,2)
    matlabbatch{X}.spm.stats.fmri_est.spmmat = {fullfile(parent_dir,members{z},['results_representation_',num2str(trial_num),'_DCM'] ,'SPM.mat')};  %%
