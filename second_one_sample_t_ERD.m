...
savename = ['tri_',num2str(trial_num),'_ERD_one_t.mat']; %%

for c=1:6
matlabbatch{X}.spm.stats.factorial_design.dir = {fullfile(parent_dir,['group_analysis_one_sample_t_',num2str(trial_num),'_ERD'],['contrast_' num2str(c)])};  %%
%%
for z = 1:size(members,2)
matlabbatch{X}.spm.stats.factorial_design.des.t1.scans{z,1} = fullfile(parent_dir,members{z},['results_representation_',num2str(trial_num),'_ERD'],['con_00' num2str(c,'%02u') '.nii']);
...
for c = 1:6
matlabbatch{X}.spm.stats.fmri_est.spmmat = {fullfile(parent_dir,['group_analysis_one_sample_t_',num2str(trial_num),'_ERD'],['contrast_' num2str(c)],'SPM.mat')};
